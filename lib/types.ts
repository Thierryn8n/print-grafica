export type UserRole = 'admin' | 'designer' | 'gerente' | 'producao' | 'financeiro' | 'vendedor' | 'cliente'
...
// Sport-specific data interfaces
export interface FutebolData {
  posicao_em_campo?: string // Goleiro, Zagueiro, Lateral, Volante, Meia, Atacante
  numero_fixo?: boolean
  perna_dominante?: string // Direita, Esquerda, Ambidestra
}

export interface BasqueteData {
  altura?: string // em cm
  posicao?: string // Armador, Ala, Ala-pivô, Pivô
  numero_fixo?: boolean
}

export interface VoleiData {
  funcao?: string // Levantador, Oposto, Ponteiro, Líbero, Central
  numero_fixo?: boolean
}

export interface HandebolData {
  posicao?: string // Goleiro, Ponta, Lateral, Pivô, Armador
  numero_fixo?: boolean
}

export interface CorridaData {
  numero_peito?: string
  categoria?: string // 5K, 10K, Meia Maratona, Maratona
}

export interface CiclismoData {
  numero_peito?: string
  categoria?: string // Estrada, MTB, BMX
}
...
export const ASSET_CATEGORIES: { key: AssetCategory; label: string }[] = [
  ...
  { key: 'vetores', label: 'Vetores' },
  { key: 'fontes', label: 'Fontes' },
  { key: 'mockups', label: 'Mockups' },
  { key: 'fotos', label: 'Fotos' },
  { key: 'arquivos_producao', label: 'Arquivos de Produção' },
...
export const COMPANY_PLANS = [
  { key: 'free' as CompanyPlan, label: 'Free', users: 2, orders: 50, storage: 1 },
  { key: 'pro' as CompanyPlan, label: 'Pro', users: 10, orders: 500, storage: 10 },
  { key: 'enterprise' as CompanyPlan, label: 'Enterprise', users: -1, orders: -1, storage: -1 },
]
